...
class LinkedinProfileProcessor
  ...
  def initialize(api_key)
    @api_key = api_key
    @linkedin_service = LinkedinUserInfo.new(api_key)
  end
  ...
  def extract_username(url)
    ...
    match = LINKEDIN_URL_PATTERN.match(url)
    match ? match[1] : nil
  end

  def process_profile(username)
    existing_profile = LinkedinProfile.find_by(username: username)
    return existing_profile if existing_profile&.updated_at&.> 24.hours.ago

    response = @linkedin_service.fetch_user_info("https://linkedin.com/in/#{username}")
  ...
  def save_profile(data)
    profile = LinkedinProfile.find_or_initialize_by(username: data[:username])
    profile.update(
